...
function Fire({ color, ...props }) {
  ...
  useFrame((state) => {
    ...
    ref.current.material.uniforms.time.value = state.clock.elapsedTime * 20
...
const Spectrum3D = ({isAscending}) => {
  const ref = useRef();
  const { nodes } = useLoader(GLTFLoader, '/rocket.glb');

  useFrame(( {clock }) => {
    ref.current.rotation.z = clock.getElapsedTime() * 3;
  });
  ...
  return (
    <>
      <mesh 
        ...
        >
        <primitive  object={nodes["Cylinder"]} opacity={0}/>
      ...
      </mesh>
      {
        ...
        <mesh  
          ...
          rotation={[Math.PI, 0, 0]}>
            <Fire scale={2.8} isAscending={isAscending}/>
        </mesh>
      }
    </> 
  )
